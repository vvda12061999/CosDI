...
/**
 * Turns `@generateToken` tags on interfaces into runtime tokens.
 *
 ...
 * produced, in the place the config asks for.
 */
...
const TAG = /@(?:generateToken|createToken)\b[ \t]*(?:\([ \t]*['"]([^'"]+)['"][ \t]*\))?/;
const TAG_TEXT = /@(?:generateToken|createToken)\b/;
...
function findTagged(text) {
    ...
    for (const comment of comments) {
        ...
        if (target.error) {
            warnings.push('@generateToken skipped: ' + target.error);
            continue;
        }
        if (hasBinding(masked, target.name)) {
            warnings.push('@generateToken skipped: ' + target.name + ' already has a value declaration');
            continue;
        }
...
const INCLUDES = ['tagged', 'exported'];
const DEFAULT_CONFIG = {
    roots: ['assets'],
    exclude: [],
    mode: 'package',
    out: 'assets/cosdi-tokens.generated.ts',
    keysOut: 'cosdi-service-keys.d.ts',
...
function loadConfig(projectRoot) {
    ...
    let defaultOut = DEFAULT_CONFIG.out;
    if (mode === 'package') {
        defaultOut = path.join('node_modules', packageName);
    } else if (mode === 'keys') {
        defaultOut = DEFAULT_CONFIG.keysOut;
    }
...
const EXPORTED_INTERFACE = /(?:^|\n)[ \t]*export[ \t]+interface[ \t]+([A-Za-z_$][\w$]*)([^{;]*)\{/g;

/**
 * Finds the exported interfaces a key can be built from. Generic interfaces
 * are left out: each type argument would need a key of its own.
 */
function findExportedInterfaces(text) {
    const { masked } = maskCode(text);
    const names = [];
    EXPORTED_INTERFACE.lastIndex = 0;
    let match = EXPORTED_INTERFACE.exec(masked);
    while (match) {
        if (match[2].indexOf('<') < 0) {
            names.push(match[1]);
        }
        match = EXPORTED_INTERFACE.exec(masked);
    }
    return names;
}

/**
 * Builds the declaration that gives every key its type. It is a module
 * augmentation, so nothing imports it, and it holds no runtime code.
 */
function buildKeysModule(entries, out, importFrom, eol) {
    const lines = ['// Generated by CosDI. Do not edit.'];
    for (const entry of entries) {
        lines.push('import type { ' + entry.name + ' as ' + entry.alias + " } from '"
            + importSpecifier(out, entry.file) + "';");
    }
    lines.push('');
    lines.push("declare module '" + importFrom + "' {");
    lines.push('    interface ServiceTypes {');
    for (const entry of entries) {
        lines.push("        '" + entry.keyName + "': " + entry.alias + ';');
    }
    lines.push('    }');
    lines.push('}');
...
function buildTokensModule(entries, out, importFrom, eol) {
    const lines = [
        '// Generated by CosDI from @generateToken interfaces. Do not edit.',
...
function generateInline(files, options, result) {
    for (const file of files) {
        ...
        result.scanned++;
        if (!TAG_TEXT.test(source) && source.indexOf(MARKER) < 0) {
            continue;
        }
...
function collectEntries(files, options, result) {
    ...
    for (const file of files) {
        ...
        for (const target of found.targets) {
            if (seen[target.name]) {
                result.warnings.push(file + ': @generateToken skipped: ' + target.name
...
function generateToPackage(files, options, result) {
    ...
    writeIfChanged(manifest, JSON.stringify({
        ...
        version: '0.0.0',
        private: true,
        description: 'Generated by CosDI from @generateToken interfaces. Do not edit.',
        main: './index.ts',
        module: './index.ts',
        types: './index.ts',
        exports: { '.': './index.ts' },
...
/**
 * `keys` mode: the interface name is the key, so nothing is written next to
 * the interface. One declaration file maps each key to its type, which keeps
 * `resolve('IExampleService')` typed without giving the interface a value.
 */
function generateToKeys(files, options, result) {
    const entries = [];
    const seen = Object.create(null);
    const tagged = (options.include || 'tagged') === 'tagged';

    for (const file of files) {
        const source = fs.readFileSync(file, 'utf8');
        result.scanned++;
        if (tagged && !TAG_TEXT.test(source) && source.indexOf(MARKER) < 0) {
            continue;
        }

        // A key needs no value, so any inline token left by another mode goes.
        const stripped = stripGenerated(source);
        if (stripped !== source) {
            result.changed.push(file);
            if (!options.check) {
        ...
        let found;
        if (tagged) {
            const scan = findTagged(stripped);
            for (const warning of scan.warnings) {
                result.warnings.push(file + ': ' + warning);
            }
            found = scan.targets.map((target) => ({ name: target.name, keyName: target.tokenName }));
        } else {
            found = findExportedInterfaces(stripped).map((name) => ({ name, keyName: name }));
        }
        if (!found.length) {
            continue;
        }
        ...
        for (const target of found) {
            if (seen[target.keyName]) {
                result.warnings.push(file + ': key skipped: ' + target.keyName
                    + ' is already mapped from ' + seen[target.keyName]);
                continue;
            }
            seen[target.keyName] = file;
            entries.push({ name: target.name, keyName: target.keyName, alias: target.name + '_', file });
            result.tokens++;
        }
    }

    entries.sort((a, b) => (a.keyName < b.keyName ? -1 : 1));

    if (!entries.length) {
        remove(options.out, options.check, result);
        return;
    }
    writeIfChanged(options.out, buildKeysModule(entries, options.out, options.importFrom || 'cosdi', '\n'), options.check, result);
}
...
function generateTokens(options) {
    ...
    if (settings.mode === 'keys') {
        generateToKeys(files, settings, result);
    } else if (settings.mode === 'package') {
